...
interface AlarmsSchwarzTableWidgetSettings extends TableWidgetSettings {
  alarmsTitle: string;
...
interface AlarmSchwarzWidgetActionDescriptor extends TableCellButtonActionDescriptor {
  details?: boolean;
  acknowledge?: boolean;
  clear?: boolean;
  activity?: boolean;
...
@Component({
  selector: 'tb-alarms-schwarz-table-widget',
  templateUrl: './alarms-schwarz-table-widget.component.html',
  styleUrls: ['./alarms-schwarz-table-widget.component.scss', './../table-widget.scss']
})
export class AlarmsSchwarzTableWidgetComponent extends PageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public alarmsDatasource: AlarmsSchwarzDatasource;
  ...
  private settings: AlarmsSchwarzTableWidgetSettings;
  ...
  public newData: AlarmDataInfo[] = [];

  public newDataSource = new MatTableDataSource(this.newData);

  @ViewChild('colFilterList', {static: true})
  colFilterList: MatSelectionList;
  ...
  private updateAlarmSource() {
    ...
    const actionCellDescriptors: AlarmSchwarzWidgetActionDescriptor[] = [];
      ...
      actionCellDescriptors.push(
        {
          ...
          icon: 'comment',
          activity: true
        } as AlarmSchwarzWidgetActionDescriptor
      );
    }
    ...
    this.alarmsDatasource = new AlarmsSchwarzDatasource(this.subscription, latestDataKeys, this.ngZone, this.ctx, actionCellDescriptors);
    this.alarmsDatasource.alarmsSubject.subscribe(alarms => {
      this.newData = alarms;
      this.newDataSource = new MatTableDataSource(this.newData);
      console.log('alarmsSubject', alarms);
  ...
  private updateData() {
    ...
    this.ctx.detectChanges();
    this.alarmsDatasource.alarmsSubject.subscribe(v => {
      console.log('v', v);
      // this.newData = v.data;
    });
    if (this.alarmsDatasource.subscription.alarms?.data?.length) {
      let obj = {};

      this.alarmsDatasource.subscription.alarms.data.forEach(item => {
        Object.entries(item).forEach(([key, value]) => {
          if (value !== undefined) { // ignore undefined values
            if (!obj[key]) {
              obj[key] = [];
            }
            // check if the value is already in the array for the key to avoid duplicates
            if (!obj[key].find( o => o.text === value )) {
              obj[key].push({text: value, selected: false});
            }
          }
        });
      });
  ...
  public onActionButtonClick($event: Event, alarm: AlarmDataInfo, actionDescriptor: AlarmSchwarzWidgetActionDescriptor) {
  ...
  public actionEnabled(alarm: AlarmDataInfo, actionDescriptor: AlarmSchwarzWidgetActionDescriptor): boolean {
  ...
  handleSelection($event: any) {
    console.log('this.colFilterList.', this.colFilterList?.options);
    console.log('handleSelection', $event)
  }

  shownElements(name: string) {
    // console.log('colFilterList.', this.colFilterList?.options);
    // const result = [];
    // const data = this.alarmsDatasource.subscription.alarms.data;
    // for (let i = 0; i < data.length; i++) {
    //   const el = data[i];
    //   if (isDefinedAndNotNull(el[name])) {
    //     result.push({
    //       text: el.name,
    //       active: false
    //     })
    ...
    // return result;
    return this.newData ? (this.newData[name] || []) : [];
  }

  selectStart(eve) {
    console.log('selectStart', eve)
  }

  select(eve) {
    console.log('select', eve)
  }

}

class AlarmsSchwarzDatasource implements DataSource<AlarmDataInfo> {

  public alarmsSubject = new BehaviorSubject<AlarmDataInfo[]>([]);
  public pageDataSubject = new BehaviorSubject<PageData<AlarmDataInfo>>(emptyPageData<AlarmDataInfo>());
  ...
  constructor(public subscription: IWidgetSubscription,
              private dataKeys: Array<DataKey>,
              private ngZone: NgZone,
              private widgetContext: WidgetContext,
              actionCellDescriptors: AlarmSchwarzWidgetActionDescriptor[]) {
  ...
  loadAlarms(pageLink: AlarmDataPageLink, sortOrderLabel: string, keyFilters: KeyFilter[]) {
    ...
    console.log('this.subscription.alarms', this.subscription.alarms);
  }
